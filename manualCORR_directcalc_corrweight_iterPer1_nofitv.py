...
with open(file, 'r') as file:
	...
	header = next(csvreader)
	for row in csvreader:
		timestamp = row[2] + "-" + row[3] + "-" + row[4] + "_" + row[5] + "-" + row[6] + "-" + row[7] + "_" + row[13]
		channel = row[1]
		timing[timestamp][channel] = row[12]	#time in sample
		corrfocal[timestamp][channel] = row[11]	#correlation value with the focal channel
		if int(row[12])== 0:
			tstart[timestamp] = row[10]

print("#timestamp\tvStart(m/s)\tx(mm)\ty(mm)\tz(mm)\tt0(s)\tdt0(s)\tv(m/s)\tGoodnessOfFit/n\tnUsed")
